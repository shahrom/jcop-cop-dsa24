...
export default function OperationList(props) {
  ...
  const fetchOpList = () => {
    ...
    var url = `data/OpView/List/OperationList-JCOP.json`;
    ...
    fetch(url, {
      ...
      .then((a1) => {
        // 1) Get the fixed-site list for display purposes
  ...
  const connectToSocketServer = (orgName) => {
    ...
    window.SocketIO.SubscribeSocketEvent("Main", (msg) => {
      ...
      if (obj.blue_track_type_id !== undefined) {
        ...
        // Set default value for image exist so that allocated is forced to display
        obj.isImageExist = true;
  ...
  const Header = () => (
    <div>
      ...
      >
        <span style={{ color: "gray", fontSize: 24 }}>NEGERI</span>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 20, marginTop: -100 }}>
      <Header />
      <div style={{ overflow: "auto", height: window.innerHeight - 225 }}>
